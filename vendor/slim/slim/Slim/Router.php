...
class Router implements RouterInterface
{
    ...
    public function setBasePath($basePath)
    {
        if (!is_string($basePath)) {
            throw new InvalidArgumentException('O basePath do roteador deve ser uma string');
        }
    ...
    public function setCacheFile($cacheFile)
    {
        if (!is_string($cacheFile) && $cacheFile !== false) {
            throw new InvalidArgumentException('O arquivo de cache do roteador deve ser uma string ou false');
        }

        if ($cacheFile && file_exists($cacheFile) && !is_readable($cacheFile)) {
            throw new RuntimeException(
                sprintf('O arquivo de cache do roteador `%s` não é legível', $cacheFile)
            );
        }

        if ($cacheFile && !file_exists($cacheFile) && !is_writable(dirname($cacheFile))) {
            throw new RuntimeException(
                sprintf('O diretório do arquivo de cache do roteador `%s` não é gravável', dirname($cacheFile))
            );
        }
    ...
    public function map($methods, $pattern, $handler)
    {
        if (!is_string($pattern)) {
            throw new InvalidArgumentException('O padrão de rota deve ser uma string');
        }
    ...
    public function getNamedRoute($name)
    {
        ...
        throw new RuntimeException('Rota nomeada não existe para o nome: ' . $name);
    }
    ...
    public function lookupRoute($identifier)
    {
        if (!isset($this->routes[$identifier])) {
            throw new RuntimeException('Rota não encontrada, parece que o seu cache de rotas está obsoleto.');
        }
    ...
    public function relativePathFor($name, array $data = [], array $queryParams = [])
    {
        ...
        if (empty($segments)) {
            throw new InvalidArgumentException('Dados ausentes do segmento de URL: ' . $segmentName);
        }
