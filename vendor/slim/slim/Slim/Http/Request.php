...
class Request extends Message implements ServerRequestInterface
{
        ...
        if (isset($this->bodyParsers[$mediaType])) {
            ...
            if (!is_null($parsed) && !is_object($parsed) && !is_array($parsed)) {
                throw new RuntimeException(
                    'O valor de retorno do analisador do tipo de mídia do corpo da solicitação deve ser uma matriz, um objeto ou um valor nulo'
                );
            }
    ...
    public function withParsedBody($data)
    {
        if (!is_null($data) && !is_object($data) && !is_array($data)) {
            throw new InvalidArgumentException('O valor do corpo analisado deve ser uma matriz, um objeto ou um valor nulo');
        }
