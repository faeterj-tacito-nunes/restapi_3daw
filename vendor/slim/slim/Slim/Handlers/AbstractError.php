...
abstract class AbstractError extends AbstractHandler
{
    ...
    protected function writeToErrorLog($throwable)
    {
        ...
        $message = 'Erro de Aplicação SLIM:' . PHP_EOL;
        $message .= $this->renderThrowableAsText($throwable);
        while ($throwable = $throwable->getPrevious()) {
            $message .= PHP_EOL . 'Erro anterior:' . PHP_EOL;
            $message .= $this->renderThrowableAsText($throwable);
        }
    ...
    {
        $text = sprintf('Tipo: %s' . PHP_EOL, get_class($throwable));

        if ($code = $throwable->getCode()) {
            $text .= sprintf('Código: %s' . PHP_EOL, $code);
        }

        if ($message = $throwable->getMessage()) {
            $text .= sprintf('Mensagem: %s' . PHP_EOL, htmlentities($message));
        }

        if ($file = $throwable->getFile()) {
            $text .= sprintf('Arquivo: %s' . PHP_EOL, $file);
        }

        if ($line = $throwable->getLine()) {
            $text .= sprintf('Linha: %s' . PHP_EOL, $line);
        }

        if ($trace = $throwable->getTraceAsString()) {
            $text .= sprintf('Caminho: %s', $trace);
        }
